...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Function {DepthFilter}{$\mathbf{D_i}$, $\mathbf{S_{in}}$, $d_{range}$}
...
\begin{algorithm}
    ...
    \begin{algorithmic}[1]
        ...
        \Procedure{GetObjectPixels}{$\mathbf{D_i}$, $\mathbf{S_{hand}}$,$m_{object}$}
            \State $\mathbf{V_i} \gets$ \Call{DepthToVertex}{$\mathbf{D_i}$}
            \State $\mathbf{S_{object}} \gets$ \Call{RadiusFilter}{$\mathbf{V_i}$,$\mathbf{S_{hand}}$}
            \State $\mathbf{S_{object}} \gets$ \Call{DepthFilter}{$\mathbf{D_i}$, $\mathbf{S_{object}}$,$thd_{object}$}
            \State $\mathbf{S_{object}} \gets$ \Call{LargestConnectedDomain}{$\mathbf{S_{object}}$}
        \EndProcedure
